# -*- coding: utf-8 -*-
"""
Created on Mon May  1 20:19:39 2023
...
fig_bool=1
w_path_sig='D:\WIND_/daily2monthly/'
save_name='UV_WSC_Mm'
Mytime=['2000-01','2000-12']

U=xr.open_dataset('G:/MODEL_DATA/ERA5_model/ERA5_daily_uwind_1979_2022.nc').loc[dict(wind_time=slice(Mytime[0],Mytime[-1]),\
                                                                   lat=slice(-80,-10))]
V=xr.open_dataset('G:/MODEL_DATA/ERA5_model/ERA5_daily_vwind_1979_2022.nc').loc[dict(wind_time=slice(Mytime[0],Mytime[-1]),\
                                                                   lat=slice(-80,-10))]

YM_U=U.groupby('wind_time.month').mean()
YM_V=V.groupby('wind_time.month').mean()
...
for i,j in tqdm(zip(YM_U.Uwind,YM_V.Vwind)):
