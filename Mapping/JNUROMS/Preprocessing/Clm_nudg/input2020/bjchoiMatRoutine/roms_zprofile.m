function [out,zout] = roms_zprofile(data,grd,lonlat)
% Get vertical profiles out of 3-D ROMS variable at arbitrary horizontal
% locations 
%
% Inputs
%    data = ROMS 3-D variable dimensioned (s,lat,lon)
%    grd = grid structure (see roms_get_grid)
%    lonlat = matrix dimensioned (num_points,2) of target lon/lat
%             coordinates at which to find the vertical profiles 
%             (column 1 = longitude, column2 = latitude)
% Outputs
%    out = matrix of vertical profiles (depth,lonlat) 
%    zout = corresponding depth coordinates (depth,lonlat)
%
% John Wilkin - Mar 2001

% Actually do horizontal interpolations at each s-coordinate level

% Figure out whether the data are for the u,v, or rho points grid
...
switch var
  ...
  case 'v'
    ...
    ygrid = grd.lat_v;
  otherwise
    % already at Arakawa-C rho points
    xgrid = grd.lon_rho;
    ygrid = grd.lat_rho;
    z = grd.z_r;
end

% step though each target lon/lat point
...
for i=1:size(lonlat,1)
  ...
  % Find the nearest ROMS grid points 
  [Iindex,Jindex] = find_nearestgridpoints(xpoint,ypoint,xgrid,ygrid);
  Iindex,Jindex
  % Extract the small data subset and just use griddata to interpolate data to
  % the target location (looping through depths) (get the z levels too)
  xsub = xgrid(Iindex,Jindex);
  ysub = ygrid(Iindex,Jindex);
  datasub = squeeze(data(:,Iindex,Jindex));
  zsub = squeeze(z(:,Iindex,Jindex));

  if ~isempty(xsub)
    for k=1:grd.N
      out(k,i) = griddata(xsub,ysub,squeeze(datasub(k,:,:)),xpoint,ypoint,'linear');
      zout(k,i) = griddata(xsub,ysub,squeeze(zsub(k,:,:)),xpoint,ypoint,'linear');
    end
  else
    out(1:grd.N,i) = repmat(NaN,[grd.N 1]);
    zout(1:grd.N,i) = repmat(NaN,[grd.N 1]);
  end
...
function [Iindex,Jindex] = find_nearestgridpoints(xpoint,ypoint,xgrid,ygrid)
% For a structured grid (xgrid,grid) find the indicies Index,Jindex that
% enclose the target point xpoint,ypoint
%
% An empty result indicates the point is not located inside any grid cell.
%
% This function can be used to find the 4 data points surrounding any
% coordinate in a curvilinear grid such as used by ROMS
%
% John Wilkin - March 2001

% Find the nearest grid point to the target
dist = sqrt((xgrid-xpoint).^2+(ygrid-ypoint).^2);
[I,J] = find(dist==min(dist(:)));
I = I(1); % take element 1 in case min dist is not unique
J = J(1);

% There are four cells that include the nearest grid point
% Which cell contains the target?
try
  Iindex = [I I+1 I+1 I];
  Jindex = [J J J+1 J+1];
  xp = diag(xgrid(Iindex,Jindex));
  yp = diag(ygrid(Iindex,Jindex));
  if ~inside(xpoint,ypoint,xp,yp)    
    Iindex = [I I+1 I+1 I];
    Jindex = [J-1 J-1 J J];
    xp = diag(xgrid(Iindex,Jindex));
    yp = diag(ygrid(Iindex,Jindex));
  ...
  errstr = ['(' num2str(xpoint) ',' num2str(ypoint) ') is not in the grid']; 
  warning(errstr)
  Iindex = [];
  Jindex = [];
end
