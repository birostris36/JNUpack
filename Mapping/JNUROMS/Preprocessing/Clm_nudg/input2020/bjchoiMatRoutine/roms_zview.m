function [thedata,thegrid,han] = roms_zview(file,var,time,depth,grd,...
    contr,vec_d,uscale,varargin)
% [thedata,thegrid,han] = roms_zview(file,var,time,depth,grd,...
...
% contr if 1 draw a countour (u can modify contour line color,
%                             linwidth, 
...
%
% John Wilkin
% Peter Lee, CNU Added 23 Nov 2008
...
%----------------------peter added 22 Nov. 2008------------------------%
if nargin > 5
    if contr==1,
        % [C,h,hanpc] = contourf(x,y,data.*mask,[0:2:35]);
        % [C,h,hanpc] = contourf(x,y,data.*mask);
        % set(h,'edgecolor','w');
%----------------------------------------------------------------------%
        hold on
        % clabel(C1,h1,'LabelSpacing',1000,'Rotation',0,'Color','k')
        [hanpc,h] = contour(x,y,data);
        set(h,'edgecolor','w','linewidth',2);
%----------------------------------------------------------------------%
        clabel(hanpc,h,'fontsize',15);
        caxis(cax);
        colorbar('vert');
    else
        caxis(cax);
...
%----------------------peter added 22 Nov. 2008------------------------%
% set(gcf,'position',[600 20 1100 950])
set(gcf,'color','w','position',[150 20 1100 950])
%----------------------------------------------------------------------%
