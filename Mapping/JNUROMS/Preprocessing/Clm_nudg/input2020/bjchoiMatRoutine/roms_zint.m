...
if ( nargin == 4 )
	...
	if ( ( rz1 ~= rz ) | ( cz1 ~= cz ) )
		format = '%s:  size of z1 [%dx%d] does not match YX dimensions of grd.z_r [%dx%dx%d]\n';
		msg = sprintf ( format, mfilename, rz1, cz1, tz, rz, cz );
		error ( msg );
	end
	if ( ( rz2 ~= rz ) | ( cz2 ~= cz ) )
		format = '%s:  size of z2 [%dx%d] does not match YX dimensions of grd.z_r [%dx%dx%d]\n';
...
for r = 1:rz
	for c = 1:cz
		...
		if isempty(ind)
			...
			real_bottom_layer(r,c) = -1*grd.h(r,c);
			; 
