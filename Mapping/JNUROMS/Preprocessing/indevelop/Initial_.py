# -*- coding: utf-8 -*-
"""
Created on Sat Jan 21 14:52:53 2023
...
MyVar={'Layer_N':36,'Vtransform':1,'Vstretching':1,\
       'Theta_s':4,'Theta_b':0,'Tcline':50,'hmin':50}
...
for i in ['zeta','u','v','temp','salt']:
    ...
    if len(tmp_data.shape)==2:
        data=griddata((lonO_s_m[tmp_mask].flatten(),latO_s_m[tmp_mask].flatten()),\
                      tmp_data[tmp_mask].flatten(),(lonG.flatten(),latG.flatten()),'nearest')
        ...
        for j,k in zip(tmp_data,tmp_mask):
            data_=griddata((lonO_s_m[k].flatten(),latO_s_m[k].flatten()),\
                          j[k].flatten(),(lonG.flatten(),latG.flatten()),'nearest')
