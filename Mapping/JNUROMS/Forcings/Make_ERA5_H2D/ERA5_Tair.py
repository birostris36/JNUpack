...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 15:34:52 2023
...
save_name='ERA5_daily_tair_1979_2022.nc'

nc=Dataset(read_path+'ERA5_2temp_S_1979_2022.nc')
...
stT=time.time()
create_tair_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_tair_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    time = ncfile.createVariable('tair_time', np.float64, ('tair_time',),zlib=False)
    time.units=Ref_time
    time.field='tair_time, scalar, series'

    Tair = ncfile.createVariable('Tair',np.float64,('tair_time','lat','lon'),\
