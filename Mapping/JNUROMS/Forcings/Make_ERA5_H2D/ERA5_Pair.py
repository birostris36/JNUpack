...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 16:13:00 2023
...
save_name='ERA5_daily_pair_1979_2022.nc'

nc=Dataset(read_path+'ERA5_msl_S_1979_2022.nc')
...
for YY in [f'{i:02d}' for i in range(1979,2023)]:
    ...
    for MM in [f'{i:02d}' for i in range(1,13)]:
        for DD in Times_guide['Day'].loc[(Times_guide['Year']==YY)&(Times_guide['Month']==MM)]:
            ...
            MY_Var[N]=np.concatenate([posi_var,nega_var],axis=1) / 100
            N+=1
...
stT=time.time()
create_pair_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_pair_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    ncfile.title='ERA5 daily pair'
    
    lat = ncfile.createVariable('lat', np.float32, ('lat',),zlib=False)
    lat.units = 'degrees_north'
    lon = ncfile.createVariable('lon', np.float32, ('lon',),zlib=False)
    lon.units = 'degrees_east'
    time = ncfile.createVariable('pair_time', np.float64, ('pair_time',),zlib=False)
    time.units=Ref_time
    time.field='pair_time, scalar, series'

    Pair = ncfile.createVariable('Pair',np.float64,('pair_time','lat','lon'),\
