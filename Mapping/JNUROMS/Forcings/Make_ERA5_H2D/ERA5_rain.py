...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 14:42:27 2023
...
save_name='ERA5_daily_rain_1979_2022.nc'

nc=Dataset(read_path+'ERA5_tp_S_1979_2022.nc')
...
stT=time.time()
create_rain_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_rain_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    time = ncfile.createVariable('rain_time', np.float64, ('rain_time',),zlib=False)
    time.units=Ref_time
    time.field='rain_time, scalar, series'

    Uwind = ncfile.createVariable('rain',np.float64,('rain_time','lat','lon'),\
                                zlib=False, fill_value=-32767) #
    Uwind.units =  'kg mm-2 s-1'
    Uwind.long_name = 'rain fall rate' # this is a CF standard name
    Uwind.time='rain_time'
    Uwind.coordinates = "lon lat"
    Uwind.field='rain, scalar, series'
