...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 14:58:52 2023
...
save_name='ERA5_daily_cloud_1979_2022.nc'

nc=Dataset(read_path+'ERA5_tc_S_1979_2022.nc')
...
stT=time.time()
create_cloud_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_cloud_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    time = ncfile.createVariable('cloud_time', np.float64, ('cloud_time',),zlib=False)
    time.units=Ref_time
    time.field='cloud_time, scalar, series'

    Cloud = ncfile.createVariable('Cloud',np.float64,('cloud_time','lat','lon'),\
                                zlib=False, fill_value=1e+20) #
    Cloud.units =  'nondimensional'
    Cloud.long_name = 'cloud' # this is a CF standard name
