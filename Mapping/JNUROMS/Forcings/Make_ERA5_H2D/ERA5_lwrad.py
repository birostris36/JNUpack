...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 16:36:35 2023
...
save_name='ERA5_daily_lwrad_1979_2022.nc'

nc=Dataset(read_path+'ERA5_surface_termal_rad_down_S_1979_2022.nc')
...
for YY in [f'{i:02d}' for i in range(1979,2023)]:
    ...
    for MM in [f'{i:02d}' for i in range(1,13)]:
        for DD in Times_guide['Day'].loc[(Times_guide['Year']==YY)&(Times_guide['Month']==MM)]:
            ...
            MY_Var[N]=np.concatenate([posi_var,nega_var],axis=1) / 3600
            N+=1
...
stT=time.time()
create_lwrad_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_lwrad_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    time = ncfile.createVariable('lrf_time', np.float64, ('lrf_time',),zlib=False)
    time.units=Ref_time
    time.field='lrf_time, scalar, series'

    lwrad_down = ncfile.createVariable('lwrad_down',np.float64,('lrf_time','lat','lon'),\
