...
lat_new = np.arange(lat_rng[0],lat_rng[-1], 1)
lon_new = np.arange(lon_rng[0],lon_rng[-1], 1)
...
def Plot_SO_Merc2(MyDATA_uv,MyDATA_scalar,My_levels,CMAP,Mylim,w_path,save_name,fig_bool=False):
    ...
    ax.add_feature(cf.COASTLINE.with_scale("10m"), lw=1,zorder=110)
    ax.add_feature(cf.LAND.with_scale("10m"),color=[.75,.75,.75],zorder=100)

    q = plt.quiver(MyDATA_uv['lon'],MyDATA_uv['lat'],MyDATA_uv['u'],MyDATA_uv['v'],
        scale=6,headwidth=7.5,headaxislength=10,headlength=13,color='k',
        minlength=1,edgecolor='y',minshaft=1.3,alpha=.7,transform=PC,zorder=1)

    # M=plt.contourf(lonA,latA,MyDATA_scalar,cmap=CMAP,levels=My_levels,transform=PC,zorder=0)
