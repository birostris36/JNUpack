...
wpth='C:/Users/shjo/OneDrive/mySO/zonal_mean_Atlantic/'
...
print('!!! Open files !!!')
myCoefs,myNm,myLat=[],[],[]
lat_rng=[-75,-30]; time_rng=[str(t_rng[0])+'-01',str(t_rng[-1])+'-12']
lon_rng1=[-70+360,360] ; lon_rng2=[0,20]  # Atlantic 

for i in myDATA: 
    print('!!! Open: '+i+' !!!')
    tmp=xr.open_dataset(i)
    if len(tmp.coords)==3:
        mySST1 = tmp[varnm].loc[dict(lat=slice(lat_rng[0],lat_rng[-1]),lon=slice(lon_rng1[0],lon_rng1[-1]),\
            time=slice(time_rng[0],time_rng[-1]))]
        mySST2 = tmp[varnm].loc[dict(lat=slice(lat_rng[0],lat_rng[-1]),lon=slice(lon_rng2[0],lon_rng2[-1]),\
            time=slice(time_rng[0],time_rng[-1]))]
        
    else:
        mySST1 = tmp['temp'].loc[dict(lat=slice(lat_rng[0],lat_rng[-1]),lon=slice(lon_rng1[0],lon_rng1[-1])\
    ,time=slice(time_rng[0],time_rng[-1]),depth=slice(0,10))].mean(dim='depth')
        mySST2 = tmp['temp'].loc[dict(lat=slice(lat_rng[0],lat_rng[-1]),lon=slice(lon_rng2[0],lon_rng2[-1])\
    ,time=slice(time_rng[0],time_rng[-1]),depth=slice(0,10))].mean(dim='depth')
        
    mySST=xr.concat([mySST1,mySST2],dim='lon')
...
axs.set_title(myName+'\nAtlantic '+str(t_rng[0])+' '+str(t_rng[-1]),loc='right',fontdict={'fontsize':20,'fontweight':'regular','fontstyle':'normal'})
...
if fig_bool:
    myName.replace(' ','_')
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_Atlantic'+'_ensembled_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
...
if fig_bool:
    myName.replace(' ','_')
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_Atlantic_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
