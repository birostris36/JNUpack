...
wpth='C:/Users/shjo/OneDrive/mySO/zonal_mean_SCP/'
...
print('!!! Open files !!!')
myCoefs,myNm,myLat=[],[],[]
lat_rng=[-75,-30]; lon_rng=[180,240]; time_rng=[str(t_rng[0])+'-01',str(t_rng[-1])+'-12']
for i in myDATA: 
    print('!!! Open: '+i+' !!!')
...
if fig_bool:
    myName.replace(' ','_')
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_SCP'+'_ensembled_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
...
if fig_bool:
    myName.replace(' ','_')
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_SCP_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
