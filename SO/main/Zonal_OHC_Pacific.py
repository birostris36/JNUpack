...
wpth='C:/Users/shjo/OneDrive/mySO/zonal_mean_Pacific/'
...
print('!!! Open files !!!')
myCoefs,myNm,myLat=[],[],[]
lat_rng=[-75,-30]; lon_rng=[153,290];  time_rng=[str(t_rng[0])+'-01',str(t_rng[-1])+'-12']
for i in myDATA: 
    print('!!! Open: '+i+' !!!')
...
axs.set_title(myName+'\nPacific '+str(t_rng[0])+' '+str(t_rng[-1]),loc='right',fontdict={'fontsize':20,'fontweight':'regular','fontstyle':'normal'})
...
if fig_bool:
    ...
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_Pacific'+'_ensembled_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
...
if fig_bool:
    ...
    # plt.savefig(wnpth'/ppt/'+save_name,
    #         facecolor='none',edgecolor='none',bbox_inches='tight',transparent=True)
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_Pacific_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
