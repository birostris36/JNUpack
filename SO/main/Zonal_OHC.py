...
wpth='C:/Users/shjo/OneDrive/mySO/zonal_mean/'
...
for i in myDATA: 
    ...
    mySST = tmp[varnm].loc[dict(lat=slice(lat_rng[0],lat_rng[-1])\
...
for i,j,k in zip(myCoefs,myNm,myLat):
    axs.plot(i,k,linewidth=1,color='grey',zorder=0,alpha=1.)
...
axs.set_title(myName+'\nGlobal'+str(t_rng[0])+' '+str(t_rng[-1]),loc='right',fontdict={'fontsize':20,'fontweight':'regular','fontstyle':'normal'})
...
if fig_bool:
    ...
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_ensembled_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
...
fig, axs = plt.subplots(1,1,figsize=(4.5,6.5),constrained_layout = True,dpi=200)
for i,j,k in zip(myCoefs,myNm,myLat):
    axs.plot(i,k,label=j,linewidth=2,zorder=0,alpha=0.7,color=dta_colr(j))
axs.axhline(y=-55,color='k',linestyle='-.')
axs.axhline(y=-60,color='k',linestyle='-.')
axs.axvline(x=0,color='k',linestyle='-')
axs.set_title(myName+'\nGlobal'+str(t_rng[0])+' '+str(t_rng[-1]),loc='right',fontdict={'fontsize':20,'fontweight':'regular','fontstyle':'normal'})
...
if fig_bool:
    ...
    # plt.savefig(wnpth'/ppt/'+save_name,
    #         facecolor='none',edgecolor='none',bbox_inches='tight',transparent=True)
    plt.savefig(wpth+'/'+myName.replace(' ','_')+'_'+str(t_rng[0])+'_'+str(t_rng[-1]),bbox_inches='tight')
plt.show()
