...
def myCrtpy_sph_pcolor(LAT,LON,DATA,CMAP,mylevel,myName,wpth):
    ...
    M=plt.pcolormesh(LON,LAT,DATA,cmap=CMAP,transform=PC,vmin=-0.005,vmax=0.005)
...
mydata={'myEofs':myEofs,'myPcs':myPcs,'myNm':myNm,'myLat':myLat,'myVar':myVar,'myVar2':myVar2}
with open(snm+'/EOF_ERA5_'+wpth.split('/')[-2]+'.pickle', 'wb') as f:
