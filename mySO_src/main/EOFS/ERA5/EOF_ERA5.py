...
pthMO='D:/ERA5_monthly_85/'
# pthrn='J:/Reanalysis/'

wpth='C:/Users/shjo/OneDrive/mySO/EOFS/ERA5_Pair/'
...
tmp=xr.open_dataset('D:/ERA5_monthly_85/ERA5_monthly_pair_1980_2021.nc')

myDATA = tmp[varnm].loc[dict(lat=slice(lat_rng[0],lat_rng[-1]),lon=slice(lon_rng[0],lon_rng[-1]),\
    pair_time=slice(time_rng[0],time_rng[-1]))]

myDATA=myDATA.where(myDATA<10**30)


time,latR,lonR=myDATA.pair_time.values,myDATA.lat.values,myDATA.lon.values
dta_nm='ERA5 '+varnm+' eof '+\
    str(time[0])[:4]+' '+str(time[-1])[:4]

OHC_1Y=myDATA.rolling(pair_time=12,center=True).mean()[6:-5]
...
for nm,lat_,lon_,eofs_,pcs_,var1_,var2_ in zip(myNm,myLat,myLon,myEofs,myPcs,myVar,myVar2):
    ...
    for eof,pc,var1,var2,N in zip(eofs_.values,pcs_.transpose(),var1_,var2_,range(1,len(eofs_)+1)):
        ...
        TIME= [str(i)[0:7] for i in pcs_.pair_time.values]
        TIME2=[str(i)[2:4] for i in pcs_.pair_time.values]
