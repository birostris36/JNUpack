...
### ======================================================================
npth='D:/ERA5_monthly_85/ERA5_monthly_wind_1980_2021.nc'
...
for i in myDATA: 
    ...
    if len(tmp.coords)==3:
        mydata = tmp[varnm].loc[dict(lat=slice(lat_rng[0],lat_rng[-1])\
            ,wind_time=slice(t_rng[0],t_rng[-1]))]
    ...
    mydata=mydata.rolling(wind_time=12,center=True).mean()[6:-5]
    
    lonR,latR=mydata.lon.values,mydata.lat.values
    lonR_m,latR_m=np.meshgrid(lonR,latR)
    time=mydata.wind_time.values
