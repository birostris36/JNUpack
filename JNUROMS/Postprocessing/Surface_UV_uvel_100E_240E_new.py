# -*- coding: utf-8 -*-
"""
Created on Wed May 17 20:09:26 2023
...
w_path='D:/OneDrive/base142/Model_results/TK0525ED_CLM/Surface_UV_uvel/'
save_name='Surface_UV_uvel'
...
for i in tqdm(range(len(AVGS))):
    ...
    tmp_zeta=Dataset(AVGS[i])['zeta'][:,lat_co,lon_co]
    
    tmp_u=Dataset(AVGS[i])['u_eastward'][:,-1,lat_co,lon_co]
    tmp_v=Dataset(AVGS[i])['v_northward'][:,-1,lat_co,lon_co]
...
for z,i,j,ii,jj,tt in zip(Zeta,U_posi,V_posi,U_nega,V_nega,My_date): 
    ...
    q1 = plt.quiver(lon_posi,lat_posi,i,j,i,
        scale=10, width=0.0025, headwidth=6.5, headaxislength=2.7,color='k',
    ...
    cb2.ax.yaxis.set_ticks_position('right')# -*- coding: utf-8 -*-
