...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 16:51:46 2023
...
save_name='ERA5_daily_srf_1979_2022.nc'

nc=Dataset(read_path+'ERA5_net_solar_rad_S_1979_2022.nc')
...
stT=time.time()
create_srf_nc(save_path+save_name,LON_new,LAT_new,My_time_num,My_time_ref,MY_Var)
print('\n!!! END Total Elapsed time : '+str((time.time()-stT)/60)[:4]+'min !!!')
...
def create_srf_nc(nc_save_name,LON,LAT,TIME,Ref_time,values):
    ...
    time = ncfile.createVariable('srf_time', np.float64, ('srf_time',),zlib=False)
    time.units=Ref_time
    time.field='srf_time, scalar, series'

    srf = ncfile.createVariable('srf',np.float64,('srf_time','lat','lon'),\
