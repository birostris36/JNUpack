...
My_Bry='G:/MODEL_DATA/Bry/TEST_py.nc'
My_Grd='G:/MODEL_DATA/Grd/Grd_SO_05d_sponge.nc'
OGCM_PATH='G:/SODA/'

Bry_title='TEST_BRY'
# My Variables
MyVar={'Layer_N':20,'Vtransform':2,'Vstretching':4,\
...
dz4uv=np.zeros([Bry_time_num.shape[0],len(depthO)-1,onG])
for jj in range(onG):
    dz4uv[:,:,jj]=dz

# Barotropic velocities1
ubar=np.sum(u_tmp*dz4uv[:,:,1:],axis=1)/depthO[-1]
vbar=np.sum(v_tmp*dz4uv,axis=1)/depthO[-1]
'''
...
'''
zr=np.zeros([Rzeta.shape[0],MyVar['Layer_N'],Rzeta.shape[-1]])
zw=np.zeros([Rzeta.shape[0],MyVar['Layer_N']+1,Rzeta.shape[-1]])

for i,n in zip(Rzeta,range(Rzeta.shape[0])):
    zr[n,:,:]=jr.zlevs(MyVar['Vtransform'],MyVar['Vstretching'],MyVar['Theta_s'],\
...
# dzu=rho2u_3d(dzr);
# dzv=dzr
# ubar_=np.sum(u*dzu,axis=1)/np.sum(dzu,axis=1)
# vbar_=np.sum(v*dzv,axis=1)/np.sum(dzv,axis=1)
...
ncI['u_north'][:]=u_c
ncI['v_north'][:]=v_c
ncI['ubar_north'][:]=ubar_north
ncI['vbar_north'][:]=vbar_north
ncI.close()
