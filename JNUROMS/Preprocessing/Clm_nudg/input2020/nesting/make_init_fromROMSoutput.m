function make_inti_fromROMSoutput
% ==========================================================================
...
if exist('gl')~=1
   gl = grd('ccs');         % (1) Get Larger grid information
end

if exist('gn')~=1
    gn = grd('ctz_large6'); % (2) Get Nested grid information
end

% (3) roms data file from a larger domain
in_file='/home/server/project/bjchoi/ccs2000/ocean_avg_0002.nc';
sourcestr = [ 'From Globec-CCS 2000: 3km resolution '];
...
%analysis_day=203; % July 21, 2005 0 0 0 = 203
 analysis_day=182; % June 30, 2005 0 0 0 = 203

% (5) output file, initial file for the nested grid
out_file = 'ctz_lg_init2000July01.nc';

% ================= End of Input Parameters =================================

mm=length(in_file);
ll=length( num2str(analysis_day+1) );
in_file(end-ll-2:end-3) = num2str(analysis_day+1);
clear mm ll
...
details = [ 'Quick initial file from Globec CCS 2000 ' ...
