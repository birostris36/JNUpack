...
% addpath('/data/Lab/SCHA/MODEL/ROMS839/project/JEJU_MODEL/Make_yecs/bjchoiMatRoutine/')
% disp('Choi Path is added ')
...
sourcestr = [ 'From yecs15 20level : 1/12 resolution '];

% (4) set time: time from the base_date in days.
% May  31, 2000 = 152
 analysis_day=91;  % Apr 1, 2017 4 1 0 =1 

% (5) output file, initial file for the nested grid
out_file = './JEJU2_ini_by_yecs15_M04D01.nc';
...
ocn_day = datenum(start_date) - datenum(base_date);
...
roms.time = analysis_day +ocn_day - 1; %% have to change
...
%     itempdata=interp1(extzzr(1:nl+2,kk),exttemp(1:nl+2,jj,ii),izzr(1:nn),'pchip');
%     isaltdata=interp1(extzzr(1:nl+2,kk),extsalt(1:nl+2,jj,ii),izzr(1:nn),'pchip');
%     iudata=interp1(extzzr(1:nl+2,kk),extu(1:nl+2,jj,ii),izzr(1:nn),'pchip');
%     ivdata=interp1(extzzr(1:nl+2,kk),extv(1:nl+2,jj,ii),izzr(1:nn),'pchip');
