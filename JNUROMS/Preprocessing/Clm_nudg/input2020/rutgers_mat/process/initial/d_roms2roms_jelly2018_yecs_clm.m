...
% svn $Id: d_roms2roms.m 754 2015-01-07 23:23:40Z arango $
%=========================================================================%
...
for iday = 1 : 32
    ...
    disp([' = = = Process ',num2str(iday),'/32 = = ='])
    idp = iday + 119;
% Set file names.
run('/data/Lab/SCHA/MODEL/ROMS839/project/Jellyfish/Make_yecs/rutgers_mat_sangchul/process/startup.m');

cd /data/Lab/SCHA/MODEL/ROMS839/project/Jellyfish/Make_yecs/rutgers_mat_sangchul/process

fm_path = '/data/JHMoon/model/ROMS839/project/yecs15_auto/';

NWAdata = [fm_path 'output/yecs15_avg_0',num2str(idp,'%3.3i'),'.nc'];

NWAgrid = [fm_path 'input/yecs15_grd2.nc'];

fp_path = '/data/Lab/SCHA/MODEL/ROMS839/project/JEJU_MODEL/INPUT/';
fp1_path = '/data/Lab/SCHA/MODEL/ROMS839/project/JEJU_MODEL/Make_yecs/make_clm/';
GRDname = [fp_path 'JEJU2_grd_for_yecs15.nc'];
BRYname = [fp1_path 'JEJU2_clm_partition_yecs_v2_17_0',num2str(idp,'%3.3i'),'_natural_vspline.nc'];

CREATE = true;                   % logical switch to create NetCDFf
report = false;                  % report vertical grid information

IniRec = 1;                      % NWA time record for initialization
% IniRec = 24;                      % NWA time record for initialization
...
S.N           = 16;         % number of vertical levels at RHO-points
...
method = 'natural';             % natural interpolation
% method = 'nearest';             % natural interpolation
...
%  Set attributes for "ocean_time".
  ...
  avalue = 'seconds since 1993-01-01 00:00:00';
  [status] = nc_attadd(BRYname,'units',avalue,'ocean_time');
  
  avalue = 'gregorian';
  [status] = nc_attadd(BRYname,'calendar',avalue,'ocean_time');

%  Set global attributes.

  avalue = 'JEJU2 Model, ~1/200 resolution';
  [status] = nc_attadd(BRYname,'title',avalue);

  avalue = 'ROMS yecs15 application';
  [status] = nc_attadd(BRYname,'data_source',avalue);
...
if (CREATE),
  ...
  for var = VarGrd,
    field = char(var);
    [status] = nc_write(BRYname, field, T.(field));
  end
  
  IniRec = 1;

  field = 'ocean_time';
  [err.(field)] = nc_write(BRYname, field, I.(field), IniRec);
