function [transport,los,angle0]=roms_section(file,grd);

%   [transport,los,angle0]=roms_transect(file,grd);
%
...
% Where is the gridded data?

d2r = pi/180;
r2d = 180/pi;

%get grid info if not included

if nargin < 2
  nc = netcdf(file);
  ncgrid = lower(nc.grd_file(:));
  
  if findstr(ncgrid,'nena')
    gf = '/home/paulg/roms2p1/grids/roms_nena_grid_6.nc';
  elseif findstr(ncgrid,'latte')
    gr = '/home/paulg/roms2p1/grids/roms_latte_grid_3.nc';
  else
    error('Sorry, I can only find the NENA or LATTE grids. You have to get the grid info yourself')
  end
  ...
  theta_b = nc{'theta_b'}(:);
   Tcline = nc{'Tcline'}(:);
    ...
    jtime = time+2448623; 
     ...
     if (length(jtime) > 1)
         error('Sorry, I can only deal with one time slice at a time')
     end
  
   result=close(nc);
	
  grd = roms_get_grid(gf,[theta_s theta_b Tcline N]);
...
% velocity
   ...
   udz(30,:,:) = squeeze(u(30,:,:)).*(fsu + squeeze(dzu(30,:,:)));
   vdz(30,:,:) = squeeze(v(30,:,:)).*(fsv + squeeze(dzv(30,:,:)));
...
if strcmpi(qq(1),'m');
  ...
  if isempty(f)
     ...
     figure(f)
     set(f,'Position',[300 300 900 600]);
     pcolorjw(grd.lon_rho,grd.lat_rho,grd.mask_rho_nan);pnc;
  else;
     figure(f)
  end;
