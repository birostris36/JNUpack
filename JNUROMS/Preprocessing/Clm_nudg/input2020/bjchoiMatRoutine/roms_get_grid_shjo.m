function grd = roms_get_grid_shjo(grd_file,scoord,tindex,calc_zuv)
...
%       tmp = nc_varget(grd_file,vname);
      grd.(vname)=My_grd_file{vname}(:) ; % grd = setfield(grd,vname,tmp);
    catch
...
%     hc      = nc_varget(scoord,'hc');
    ...
    theta_s = My_data{'theta_s'}(:);
    theta_b = My_data{'theta_b'}(:);
    Tcline  = My_data{'Tcline'}(:);
    N       = length(My_data{'Cs_r'}(:));
    hc  = My_data{'hc'}(:);
    Vtransform=My_data{'Vtransform'}(:);
    Vstretching=My_data{'Vstretching'}(:);   
  end
...
% svn $Id: roms_get_grid.m 393 2010-07-20 15:11:52Z wilkin $
%===========================================================================%
