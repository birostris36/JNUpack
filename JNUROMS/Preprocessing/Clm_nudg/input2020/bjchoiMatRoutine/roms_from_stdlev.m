function roms = roms_from_stdlev(lon,lat,zlev,data,grd,CgridPos,do_interp2)
...
% Inputs: 
%         standard level data:
%            data is dimensioned (time,zlev,lat,lon) 
%            lon is (lat,lon)
%            lat is (lat,lon) [like from meshgrid]
...
if do_interp2
  ...
  switch ndims(roms_stdlev) 
	...
	for k = 1:Nz   
	  ...
	  tmp = interp2(lon,lat,datawrk,romslon,romslat);
...
if isempty(roms)
  ...
  switch ndims(roms)
	...
	for i=1:Nx % x index
	  ...
	  [xa,za] = meshgrid(1:M,[-10000; -abs(zlev); 1]);
