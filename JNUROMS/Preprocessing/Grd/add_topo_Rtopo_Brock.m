function h=add_topo_Rtopo_Brock(grdname,toponame)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% add a topography (here etopo2) to a ROMS grid
%
% the topogaphy matrix is coarsened prior
% to the interpolation on gthe ROMS rid tp
...
if ~isempty(i2)
  topo=-nc{'bedrock_topography'}(j,i2);
%   topo=-nc{'ice_base_topography'}(j,i2);
% 
%   topo=-nc{'topo'}(j,i2);
...
if ~isempty(i1)
  topo=cat(2,-nc{'bedrock_topography'}(j,i1),topo);
%   topo=cat(2,-nc{'ice_base_topography'}(j,i1),topo);

end
if ~isempty(i3)
  topo=cat(2,topo,-nc{'bedrock_topography'}(j,i3));
%   topo=cat(2,topo,-nc{'ice_base_topography'}(j,i3));
