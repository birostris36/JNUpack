...
# save_pth='D:/OneDrive/base142/Warehouse02/V205701_MW_400VIC/'

# Avg_pth='G:/Models/V205701_MW_XICE/Avg/'
...
def Plot_SO_Spherical3(lonA,latA,MyDATA,t_name,CMAP,w_path,save_name,fig_bool=False):
...
def check_inputs(): #Log_npth,Avg_pth,save_pth
    ...
    global Avg_pth, Log_npth, save_pth

    DATA=[Avg_pth+i for i in os.listdir(Avg_pth)]
    Parameters=ti.Log_Manager(Log_npth).get_parameters(NN=12)
    ...
    ######################################
    print(save_pth+Log_npth.split('/')[-1].split('.')[0]+'.pptx')
    prs.save(save_pth+Log_npth.split('/')[-1].split('.')[0]+'.pptx')
...
# =============================================================================
    try:
        os.mkdir(save_pth+'Surface_mean_'+kargs['mean']+'_'+data_nm)
        os.mkdir(save_pth+'Surface_mean_'+kargs['mean']+'_'+data_nm+'/ppt')
    except:
        pass
...
def zonal_data_drift(data_nm,cmap,data_lim,**kargs):
    ...
    try:
        os.mkdir(save_pth+'Zonal_mean_'+kargs['mean']+'_'+data_nm) 
        os.mkdir(save_pth+'Zonal_mean_'+kargs['mean']+'_'+data_nm+'/ppt') 
    except:
        pass
...
def zonal_data_diff_Soda(data_nm,cmap,cmap1,data_lim,data_lim1,**kargs):
    ...
    try:
        [os.mkdir(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+i) for i in ['MODEL_SODA','SODA']]
        [os.mkdir(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+i+'/ppt') for i in ['MODEL_SODA','SODA']]
    except:
        pass
    ...
    for i,j in zip(data,data_soda):
        ...
        h = fig.colorbar(im1, ax=axs[:],label='',cax=cax,orientation="horizontal",extend='both',aspect=50)
        if fig_bool:
            plt.savefig(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+'MODEL_SODA/'+'ppt/'+s_name_S1,
                        facecolor='none',edgecolor='none',bbox_inches='tight',transparent=True)
            plt.savefig(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+'MODEL_SODA/'+s_name_S1,bbox_inches='tight')
        plt.show()
        ...
        h = fig.colorbar(im1, ax=axs[:],label='',cax=cax,orientation="horizontal",extend='both',aspect=50)
        if fig_bool:
            plt.savefig(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+'SODA/'+'ppt/'+s_name_S,
                        facecolor='none',edgecolor='none',bbox_inches='tight',transparent=True)
            plt.savefig(save_pth+'Zonal_meanDiff_'+kargs['mean']+'_'+data_nm+'_'+'SODA/'+s_name_S,bbox_inches='tight')
        plt.show()      

# Surface data soda diff
def Surface_data_Soda_diff(data_nm,lat_rng,cmap,cmap1,My_levels,My_levels1,data_lim,data_lim1,**kargs):
    global Avg_pth,Grd_npth, save_pth,fig_bool, SODA_pth
    ...
    if kargs['mean']=='ann':
        data=data_.groupby('ocean_time.year').mean().rename({'year':'ocean_time'})
        SODA=SODA_.groupby('ocean_time.year').mean().rename({'year':'ocean_time'})

    elif kargs['mean']=='season':
        data=data_.groupby('ocean_time.season').mean().rename({'season':'ocean_time'})
        SODA=SODA_.groupby('ocean_time.season').mean().rename({'season':'ocean_time'})
    ...
    # =============================================================================
    try:
        [os.mkdir(save_pth+'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+i) for i in ['MODEL_SODA','SODA']]
        [os.mkdir(save_pth+'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+i+'/ppt') for i in ['MODEL_SODA','SODA']]
    except:
        pass
    ...
    for i,j in zip(data,SODA):
        ...
        Plot_SO_Spherical2(lon_soda_m,lat_soda_m,j.values,\
                              t_name,My_levels,cmap,data_lim,save_pth+\
                              'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+'MODEL_SODA',s_name_S,fig_bool)
        Plot_SO_Merc2(lon_soda_m,lat_soda_m,j,t_name,My_levels,cmap,data_lim,\
                          save_pth+'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+'MODEL_SODA',s_name_M,fig_bool)
        # figure SODA
        Plot_SO_Spherical2(lon_rho,lat_rho,model_soda.values,\
                              t_name,My_levels1,cmap1,data_lim1,save_pth+\
                              'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+'SODA/',s_name_S1,fig_bool)
        Plot_SO_Merc2(lon_rho,lat_rho,model_soda,t_name,My_levels1,cmap1,data_lim1,\
                          save_pth+\
                          'Surface_Diff_'+kargs['mean']+'_'+data_nm+'_'+'SODA/',s_name_M1,fig_bool)
